...
Si todavía no tienes la respuesta, !En hora buena! Porque primero hay que resolver esta pregunta, ¿Acaso la pregunta anterior tiene sentido? Es decir, la palabra "extraño" significa que algo es desconocido y la palabra "precisión" significa que hay exactitud, entonces, ¿Cómo puede "algo desconocido" tener "exactitud"?

Por extraño que parezca, ciertamente somos capaces de jugar conas las palabras de formas tan dinámicas que aunque algo parezca no tener sentido, podríamos encontrarle un sentido particular. Pero, eso nos trae otra pregunta, ¿Qué tan conveniente es hacer aquello?

Ya que yo formulé la primera pregunta, y evidentemente dicha pregunta no parece ser muy coherente, estoy convencido de que lo principal sería no buscar respuestas sino sentido. Así pues, gracias a la incertidumbre del tema, le ofrezco estas preguntas para pensar (no para resolver).
...
Desde la perspectiva de la comunicación, este es el tipo de problemas que existen gracias a la ambiguedad, y es el inicio de un mar de posibilidades mayormente no deseadas. Permíteme compartir un par de ejemplos más para demostrar hasta que punto este tipo de problemas son comunes.
...
¿Cuál símbolo es un signo y cuál es una operación? Esta vez la respuesta podría no ser tan obvia, a menos que obviamente ya sepamos como lidiar con este tipo de situaciones. Pero, para los estudiantes nuevos en matemáticas la respuesta podría tener dos resultados diferentes:

  * x = 3 - (5 + 7) = 3 - 12 = -9
  * x = (3 - 5) + 7 = -2 + 7 = 5

Normalmente las reglas de matemáticas definen una prioridad de resolución de operaciones, es decir, la operación de resta debe resolverse primero que la adición, y la multiplicación primero que la resta y la adición; bajo esa definición, entonces, el segundo resultado sería válido: x = 5. Pero, ya que resolver el ejercicio no es el objetivo de esta sección, vamos a enfocarnos en resolver el problema de ambiguedad existente sobre el símbolo "+".
...
De hecho, si escribes este ejercicio tal como lo ves en una calculadora, tendrías la respuesta correcta. Ahora, solo para aclarar lo que hemos hecho hasta ahora, estoy usando el símbolo "+" para expresar explícitamente los dos conceptos que mismo símbolo contiene; el signo positivo y la operación de adición, pero, como habrás notado ya, usar el mismo símbolo para expresar ambos conceptos hace que el ejercicio sea innecesariamente más difícil de entender.

Para agregar un poco de claridad en este punto, podríamos agrupar términos y el resultado sería mucho mas evidente, pero, tendríamos que escribir mucho más, y al final no estamos resolviendo el problema de raíz todavía. Observa el mismo ejemplo usando paréntesis.

  * x = (+3) + (-5) + (+7)

Pero, como dijimos anteriormente, el signo "+" sigue siendo ambigüo, es decir aún sostiene dos significados diferentes. Definamos un símbolo diferente para la operación de adición: "⊕". El símbolo podría ser cualquier cosa, yo solo estoy sugiriendo algo que sea obviamente diferente a fin de identificarlo propiamente como operación de adición y no como signo positivo, ahora, veamos cómo este pequeño cambio afecta la lectura del ejercicio.
...
Imaginemos que hay una mujer que está embarazada por primera vez, y respondamos esta pregunta:
...
Entonces, de acuerdo a las proposiciones anteriores, las posibilidad de cualquier sistema serían las siguientes:
  ...
  * Sistemas reales - Existe conocimiento sistemático que puede ser compartido y entendido.
    ...
    * Inválido - Presenta resultados reprobados por su contraparte válida, ejemplo, alguien jugando un juego de mesa de forma equivocada, o alguien teniendo respuestas equivocadas en matemáticas.
  * Sistemas irreales - El conocimiento de esto es ampliamente aceptado como irreal, y probablemente no puede entendido ni compartido en totalidad.
...
He aquí un ejemplo, dos matemáticos se conocen por primera vez pero ellos no hablan el mismo idioma. Luego, ellos reciben una serie de ecuaciones, y se les ordena que sigan algunas reglas durante la resolución de los ejercicios. La cantidad de ecuaciones se supone que debería ayudar a reducir la probabilidad de respuestas iguales por coincidencia, así que nuestra expectativa debería ser que ambos matemáticos puedan resolver todos los ejercicios correctamente sin margen de error, es decir que si sus respuestas son idénticas, entonces, el sistema que estan usando debería ser real y válido. Pero, analisemos todos los posibles resultados:
...
El párrafo anterior implíca que nosotros pudiéramos ser parte de un sistema incluso sin saberlo, un ejemplo bastante evidente podrían ser los sistemas naturales como la lluvia, o incluso más drásticamente alguien planificando un secuestro, en ambos casos, cualquiera podría ser parte de estos sistemas sin saberlo. Pero, esto levanta la siguiente pregunta, ¿Los sistemas son inventados o descubiertos? Bueno, por un lado el sistema solar no fue inventado, solo traducimos ese conocimiento en nuestro lenguaje y así es como entendemos y reconocemos el sistema solar al día de hoy, pero, por otro lado si pensamos sobre cómo el primer carro funcionó, ese sistema no estaba esperando a ser descubierto, en lugar de ello, hubo varias planificaciones e intentos antes de llegar a una fórmula válida. Así que creo que es perfectamente válido decir que los sistemas son descubiertos e inventados. Ahora, solo para aclarar mi postura al respecto, el hecho de que algo no haya sido inventado no significa que no exista, en este punto comparto la filosofía de Aristoteles sobre la potencialidad y actualidad, es decir, ya que la no existencia no puede generar la existencia de algo, entonces, la no existencia nunca podrá generar algo, pero, todo aquello que existe ahora realmente siempre tuvo un potencial de existencia bajo, es decir que, todo lo que es inventado realmente tuvo una existencia inicial de potencial mínimo.
